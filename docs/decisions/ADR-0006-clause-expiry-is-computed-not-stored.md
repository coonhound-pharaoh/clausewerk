...
- Correctness now depends on a clock, which makes it environment-dependent and easy to get wrong —
  and currently is: the prototype pins "today" to a
  [hard-coded date](../spec-vs-implementation.md#6-expiry-is-evaluated-against-a-frozen-clock).
...
- [`open-questions §3`](../open-questions.md) — what happens when a clause expires mid-negotiation
- [`spec-vs-implementation §1`](../spec-vs-implementation.md#1-retired-and-expired-clauses-stay-in-the-manifest-driven-candidate-pool) — the manifest pass doesn't check `active`
