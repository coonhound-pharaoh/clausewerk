...
function ForgePanel({ manifest, ledger, decisions, setDecisions, setDossier, setTab, setPhase, setDossierReady, autoRun, tweaks }) {
  ...
  async function run() {
    ...
    for (let i = 0; i < all.length; i++) {
      ...
      const isBaseline = all[i].baseline;
      // Beat 1: query
      setBeat({ index: i, phase: 'query' });
      if (isBaseline) pushLine(`>>> baseline.include('${risk.category}')`);
      else pushLine(`>>> resolve('${risk.category}', '${risk.severity}')`);
      await wait(isBaseline ? 180 : 350);
      // Beat 2: resolve
      setBeat({ index: i, phase: 'resolve' });
      if (all[i].suppressed.length) {
        for (const s of all[i].suppressed.slice(0,2)) pushLine(`    ~ suppress ${s.id}`);
      }
      await wait(isBaseline ? 220 : 500);
      // Beat 3: land
      setBeat({ index: i, phase: 'land' });
      if (all[i].selected) pushLine(`    ← ${all[i].selected.id}${isBaseline ? '  [always-add]' : `  (${all[i].reason})`}`);
      else pushLine(`    ! no match — flagged`);
      built.push(all[i]);
      setDecisions([...built]);
      setStep(i + 1);
      await wait(isBaseline ? 120 : 220);
    }
    setBeat(null);
    await wait(250);
  ...
  const effectiveRisks = useMemo(() => {
    const baselineRisks = ledger.filter(c => c.alwaysInclude && c.active).map(c => ({
      category: c.cat, severity: 'Baseline',
      reason: `Baseline framework §${c.frameworkSection || '—'}`,
      baseline: true,
    }));
    return [...baselineRisks, ...manifest.risks];
  }, [manifest, ledger]);

  const total = effectiveRisks.length;
              ...
              <div key={i} className={`grid grid-cols-12 gap-3 items-center py-4 border-b hair transition-opacity ${i >= step && !done ? 'opacity-30' : ''}`}>
                <div className="col-span-3">
                  ...
                  <div className="mt-1 flex items-center gap-1.5">
                    <span className={`chip ${risk.severity==='High'?'chip-high':risk.severity==='Baseline'?'chip-std':'chip-std'}`} style={risk.baseline?{background:'color-mix(in oklch, var(--accent), transparent 85%)', color:'var(--accent)', border:'1px solid color-mix(in oklch, var(--accent), transparent 70%)'}:{}}>{risk.severity}</span>
                    {risk.baseline && <span className="font-mono text-[9px]" style={{color:'var(--mute-2)'}}>framework</span>}
                  </div>
                </div>
