...
const TABS = [
  ...
  { key:'validate',  label:'Validate',  tier:'·'  },
  { key:'dossier',   label:'Dossier',   tier:'·'  },
  { key:'negotiate', label:'Negotiate', tier:'T1.5', rule:true },
  { key:'review',    label:'Review',    tier:'FB' },
  { key:'audit',     label:'Audit',     tier:'FB' },
];
        ...
        <div className="flex items-center gap-6">
          {TABS.map((t, i) => (
            <React.Fragment key={t.key}>
              {t.rule && <span className="h-4 w-px" style={{background:'var(--line-2)'}}></span>}
              <button onClick={() => setActive(t.key)}
